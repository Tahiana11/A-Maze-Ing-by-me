    ...
    def _display_cell(self) -> None:
        ...
        for row in range(self.height):
            for col in range(self.width):
                ...
                if cell.walls["N"]:
                    self._fill_cell(
                        x0, x1, y0, y0 + WALL_THICKNESS - 1, 0xFF000000)
                if cell.walls["E"]:
                    self._fill_cell(
                        x1 - WALL_THICKNESS + 1, x1, y0, y1, 0xFF000000)
                if cell.walls["S"]:
                    self._fill_cell(
                        x0, x1, y1 - WALL_THICKNESS + 1, y1, 0xFF000000)
                if cell.walls["W"]:
                    self._fill_cell(
                        x0, x0 + WALL_THICKNESS - 1, y0, y1, 0xFF000000)
